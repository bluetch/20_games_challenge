...
func _physics_process(delta: float) -> void:
	...
	if collision:
		...
		if collider.is_in_group("paddles"):
			# 球拍中心點 = 根節點 y + 半個球拍高度
			var paddle_center_y: float = collider.global_position.y + 60.0

			# 球和球拍中心的垂直距離
			var hit_offset: float = global_position.y - paddle_center_y

			# 把偏移量縮放到大約 -1 到 1 的範圍
			var normalized_offset: float = hit_offset / 60.0
...
func _on_serve_timer_timeout() -> void:
	# 計時結束後，球才開始重新移動
	move_direction = next_serve_direction
	is_serving = false
